...
const CODE_STAMP = "2026-09-13-f8";
...
if(active && active.reachable && active.client && !active.expired && !active.notFound){
  ...
  async cmdSecurity(chat,mid,uid) {
    ...
    const lines=[
      ...
      "",
      L(lang,"برای تعمیر دستی وب‌هوک:","Manual webhook repair:"),
      "`"+String(origin||"https://<worker>").replace(/`/g,"'")+"/repair-webhook?key="+String(adminKey).replace(/`/g,"'")+"`",
      "",
      L(lang,"🚑 اگر ربات کاملاً ساکت شد (کلید در دسترس نیست):",
              "🚑 If the bot goes completely silent (key unavailable):"),
      "`"+String(origin||"https://<worker>").replace(/`/g,"'")+"/recover?token=<BOT_TOKEN>`",
    ];
    if(!d1){
      lines.push("", "⚠️ "+L(lang,
  ...
  async cmdDiagToken(chat,mid,uid) {
    ...
    if(rec){
      ...
      lines.push(
        ...
        "",
        L(lang,"این لینک را بدهید:","Share this link:"),
        "`"+(base+"/diag?t="+rec.token).replace(/`/g,"'")+"`",
        "",
        _unlim
          ? "⚠️ "+L(lang,"این توکن *بدون انقضا* است و دیپلوی دارد. هر کسی لینک را داشته باشد گزارش را می‌بیند و کد را دیپلوی می‌کند — لو رفت، همان‌جا ابطالش کن.","This token has *no expiry* and deploy rights. Anyone with the link can read the report and deploy code — revoke immediately if leaked.")
          : "⚠️ "+L(lang,"هر کسی این لینک را داشته باشد گزارش را می‌بیند. بعد از استفاده باطلش کنید.","Anyone with this link can read the report. Revoke it when done."),
      );
    } else {
      lines.push(
  ...
  async fetch(request,env,ctx) {
    ...
    if(url.pathname==="/recover"&&request.method==="GET"){
      try{
        ...
        const given=url.searchParams.get("token")||request.headers.get("X-Bot-Token")||"";
    ...
    if(url.pathname==="/diag/panelurl"&&request.method==="POST"){
      ...
      try{
        ...
        const given=url.searchParams.get("t")||request.headers.get("X-Diag-Token")||"";
    ...
    if(url.pathname==="/diag"&&(request.method==="GET"||request.method==="HEAD")){
      ...
      try{
        ...
        if(!rec || !given || !timingSafeEq(given, rec.token)){
          ...
          return deny("Unauthorized or expired. Generate a new token in the bot: تنظیمات → امنیت → توکن عیب‌یابی",401);
        }
    ...
    if(url.pathname==="/repair-webhook"&&request.method==="GET"){
      try{
        if(!(await store.isInstalled())) return jsonRes({ok:false,error:"not installed"},400);
        // 🔐 نیازمند کلید مدیریتی: ?key=...  یا هدر X-Admin-Key
      ...
      try{
        ...
        try{
          if(update && update.update_id!=null){
            ...
            if(globalThis.__updSeen.has(uidKey)) return new Response("OK",{status:200});
            globalThis.__updSeen.set(uidKey, Date.now());
            const dk="proc:upd:"+String(update.update_id);
            if(await store.cache(dk)) return new Response("OK",{status:200});
            await store.setCache(dk, true, 600);
            _updClaimKey=dk;
          }
        }catch{}
        ...
        // ✅ آپدیت با موفقیت پردازش شد — همین‌جا علامت می‌زنیم تا اگر تلگرام
        //    همان آپدیت را دوباره فرستاد (قطع شبکه / تایم‌اوت پاسخ)، دوباره
        //    اجرا نشود و پیام تکراری («در حال ساخت کانفیگ...») برای کاربر نرود.
        try{ if(update && update.update_id != null){ await store.setCache("proc:upd:"+String(update.update_id), true, 600); } }catch{}
      }catch(e){
        // ⚠️ سیاست retry:
        ...
        if(retriable){
          // تلگرام با فاصله دوباره تلاش می‌کند؛ چون هنوز کاری انجام نشده
          // بود، اجرای دوباره عارضه‌ای ندارد.
          // 🔓 claim این آپدیت را آزاد کن تا retry تلگرام واقعاً اجرا شود —
          //    (بدون این، dedupِ بالا retry را ۲۰۰ جواب می‌داد و آپدیت گم می‌شد)
          try{ if(_updClaimKey) await store.del("c:"+_updClaimKey); }catch{}
          try{ await store.pushLog({action:"webhook_retry", detail:emsg.slice(0,200), by:"system", meta:null}); }catch{}
